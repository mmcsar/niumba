-- ============================================
-- NIUMBA - CONFIGURATION SÉCURITÉ COMPLÈTE
-- Extensions, Auth Prevention, et RLS
-- ============================================
-- 
-- Ce script configure TOUTES les fonctionnalités de sécurité
-- disponibles dans Supabase pour votre projet
-- ============================================

-- ============================================
-- 1. EXTENSIONS DE SÉCURITÉ POSTGRESQL
-- ============================================

-- Extension pour UUID (déjà activée généralement)
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Extension pour cryptographie (hachage de mots de passe)
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Extension pour fonctions de sécurité
CREATE EXTENSION IF NOT EXISTS "pg_stat_statements";

-- ============================================
-- 2. AUTH PREVENTION (Protection Auth)
-- ============================================

-- Fonction pour empêcher les attaques par force brute
-- Limite les tentatives de connexion
CREATE OR REPLACE FUNCTION auth.prevent_brute_force()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  ...
  -- Bloquer si plus de 5 tentatives en 15 minutes
...
CREATE OR REPLACE FUNCTION auth.validate_user_input(input_text TEXT)
RETURNS BOOLEAN AS $$
BEGIN
  -- Vérifier les caractères dangereux
  IF input_text ~* '(;|--|/\*|\*/|xp_|exec|union|select|insert|update|delete|drop|create|alter)' THEN
    RETURN FALSE;
  END IF;
  RETURN TRUE;
END;
...
CREATE OR REPLACE FUNCTION auth.mask_email(email TEXT)
RETURNS TEXT AS $$
BEGIN
  ...
  -- Masquer l'email (ex: user@example.com -> u***@example.com)
...
-- ============================================
-- 5. RLS (Row Level Security) - ACTIVATION
-- ============================================
...
-- ============================================
-- 6. POLICIES RLS (Sécurité au niveau des lignes)
-- ============================================
...
DO $$
BEGIN
  RAISE NOTICE '✅ Extensions de sécurité activées !';
